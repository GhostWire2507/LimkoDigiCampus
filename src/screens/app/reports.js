...
import { getReportsForRole, updateReportFeedback } from "../../services/dataService";
...
function SeniorLecturerFeedbackForm({ report, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    const scopedReports = nextReports.filter((item) =>
      user.role === "fmg" ? true : item.programmeId === report.programmeId || item.classId === report.classId
    );
    onSaved(scopedReports);
...
function ReportsScreen() {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false}>
      <AppHeader title="Lecture Reports" subtitle="Track reporting flow from lecturer submission to leadership review." />
